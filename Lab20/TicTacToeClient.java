...
public class TicTacToeClient extends Client
{
    ...
    public void process(String message)
    {
        ...
        if("youare".equals(parts[0]))
        {
            ...
            TicTacToePiece now;//4. create a new Mayflower object with the stage you just created. This will open the GUI and start the game
            if(parts[1].equals("X"))
            {
                now= TicTacToePiece.X;
              }
            else
            {
                now= TicTacToePiece.O;
            }
            game= new TicTacToe();
            stage = new TicTacToeStage(this, game, now);
            Mayflower latte = new Mayflower("TicTacToe", 800, 600, stage);
